...
write.csv(data.smeb, file = 'Outputs/median_district_SMEB_result_jan.csv', row.names = FALSE)
...
data.smeb.natmed <- data.smeb %>% 
                    group_by(index) %>%
                    summarise_at(funs(median), .vars=vars(smeb_soap:smeb_total), na.rm=TRUE)

write.csv(data.smeb.natmed, file = 'Outputs/median_national_SMEB_jan.csv', row.names = FALSE)
...
rm(list=ls())
library(readr)
df1 <- read_csv("Inputs/median_governorate_result_jan.csv")
...
write.csv(data.smeb1, file = 'Outputs/median_governorate_SMEB_result_jan.csv', row.names = FALSE)
